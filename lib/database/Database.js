...
function read(options) {
  ...
  _.each(files, function(file) {
    if(isJSONQueryDefinitionFile(file)) {
      ...
      buildAPIFunction(api, queryDefinition, DB);
    } else if(isSQLStreamDefinitionFile(file)) {
      var filePath = path.resolve(options.queryDirectory, file);
      var queryDefinition = readSQLQueryDefinitionFile(filePath);
      buildStreamFunction(api, queryDefinition, DB);
    }
  });
...
function isSQLStreamDefinitionFile(filePath) {
  return /.*\.stream.sql$/.test(filePath);
}
...
function buildStreamFunction(api, qryDef, DB) {
  ...
  api[qryDef.id] = function() {
    ...
    return DB.stream(queryData.queryString, queryData.queryArgs);
  }
}

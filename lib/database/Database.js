 ...
 *
 * See the ./queries/ folder for examples
 */

function read() {

  var queriesDir = path.resolve(__dirname, './queries/');
  var files = wrench.readdirSyncRecursive(queriesDir);

  var definitionFiles = {};
  _.each(files, function(file) {
    if(isQueryDefinitionFile(file)) {
      var filePath = path.resolve(queriesDir, file);
      var queryDefinition = readQueryDefinitionFile(filePath);
      buildAPIFunction(api, queryDefinition);
    }
  });
...
function buildAPIFunction(api, qryDef) {
  api[qryDef.id] = function() {
    ...
    DB.query(qryDef.query.join(' '), queryArgs, queryResultHandler);
  }
}
